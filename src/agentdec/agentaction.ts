...
export class AgentInit extends BaseAgent {
  public connectionRecordAliceId?: string
  public credentialDefinition?: CredDef
  public ui: BottomBar

  public constructor(walletId: string, name: string , key:string) {
    super(walletId, name , key)
    this.config.publicDidSeed = "6b8b882e2618fa5d45ee7229ca880083"
  }

  public static async build(walletId:string , name:string , key:string): Promise<AgentInit> {
    let ag = new AgentInit(walletId, name, key )
    await ag.initializeAgent()
    return ag
  }
  ...
  private async registerSchema(name : string , attributes : string[] ) {
